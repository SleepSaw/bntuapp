...
public abstract class ExcelTableCreator extends ExcelUtil implements ExcelTable {
    ...
    @Override
    public int addAllTeacherToTable(int startRow,List<Employee> employees) {
        int i = startRow;
        int count = 1;
        for (Employee e: employees) {
            Row row = sheet.createRow(i);
            addOneTeacherToTable(count,e,row);
            i++;
            count++;
        }
        return i;
    }
    public abstract void addCommonData(int rowIndex, List<Employee> employees);

    // Асбтрактный метод, который необходимо переопределить для добавления одного учителя
    @Override
    public abstract void addOneTeacherToTable(Integer number,Employee employee, Row row);
}
